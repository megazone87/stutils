...
#   threading errors.
#
#   Defines VALGRIND_CHECK_RULES_LOCAL which should be substituted in your
...
#
#   Makefile.am must contain TESTS variable to get check-TESTS targe.
#
#   This results in a "check-valgrind-local" rule being added to any Makefile.am
#   which includes "@VALGRIND_CHECK_RULES_LOCAL@" (assuming the module has been
...
AC_DEFUN([AX_VALGRIND_CHECK_RECURSIVE],[
...
.PHONY: check-valgrind-local check-valgrind-tool
'

	AC_SUBST([VALGRIND_CHECK_RULES_LOCAL])
	m4_ifdef([_AM_SUBST_NOTMAKE], [_AM_SUBST_NOTMAKE([VALGRIND_CHECK_RULES_LOCAL])])
])
